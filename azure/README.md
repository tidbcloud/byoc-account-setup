...
The queue service read permissions are required for the Azure provider to verify storage account data-plane readiness when it authenticates to storage with Azure AD instead of storage account keys. These management-plane actions allow reading queue service and queue resource metadata only. They do not grant queue message data actions such as `Microsoft.Storage/storageAccounts/queueServices/queues/messages/read`.
